...
static int msm8x16_ext_spk_pa_init(struct platform_device *pdev)
{
	...
	hp_spk_switch_gpio = of_get_named_gpio(pdev->dev.of_node,
		"qcom,hp-spk-switch-gpio", 0);
	...
	if (gpio_is_valid(hp_spk_switch_gpio)) {
		ret = gpio_request(hp_spk_switch_gpio, "hp_spk_switch_gpio");
		if (ret) {
			pr_err("%s: gpio_request failed for hp_spk_switch_gpio.\n",
				__func__);
			return -EINVAL;
		}
