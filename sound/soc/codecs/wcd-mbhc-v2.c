				...
				void *data)
{
	...
	case WCD_EVENT_PRE_MICBIAS_2_ON:
		...
		//Modify by lichuangchuang for headset no button report in spk call mode (8916) SW00000000 2014-09-01 start
		...
		//Modify by lichuangchuang for headset no button report in spk call mode (8916) SW00000000 2014-09-01 end
		break;
	/* MICBIAS usage change */
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		//Add by lichuangchuang for headset type chenge err (8916) SW00000000 2014-08-18 start
			...
			snd_soc_update_bits(codec,
                ...
                if((pre_plug != MBHC_PLUG_TYPE_NONE) && (pre_plug != mbhc->current_plug))
                        wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
                                    0, WCD_MBHC_JACK_MASK);
		//Add by lichuangchuang for headset type chenge err (8916) SW00000000 2014-08-18 end
...
irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	//Add by lichuangchuang for headset no button report in spk call mode (8916) SW00000000 2014-09-01 start
	if(snd_soc_read(codec,MSM8X16_WCD_A_ANALOG_MICB_2_EN) & 0x80){
		snd_soc_update_bits(codec,
			MSM8X16_WCD_A_ANALOG_MICB_1_EN,
			0x40, 0x40);
	}
	//Add by lichuangchuang for headset no button report in spk call mode (8916) SW00000000 2014-09-01 end
	result1 = snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_MBHC_BTN_RESULT);
