...
#define FAKE_CHARGING_DISPLAY  // skj debug
...
#define CURRENT_1C_MASK    (0xff) 
#define CURRENT_1C_VALUE    (0x3f)  //pre:150mA  fast:3000mA
#define CURRENT_1C_VALUE_DECREASED    (0x2B)  //pre:150mA  fast:1000mA
...
static struct wake_lock smb135x_wakelock;

// by skj
#define AC_MAX_CURRENT (1400)
...
static int smb135x_charging_set(struct smb135x_chg *chip, int enable)
{
	int rc = 0;
...
// by skj
static int smb135x_get_batt_temp(struct smb135x_chg *chip)
{
#define DEFAULT_TEMP 250
	...
	if(chip->vadc_dev){
		...
		if (rc<0) {
			printk("Unable to read batt temperature rc=%d\n", rc);
			temp=DEFAULT_TEMP;
		}else{
		...
		if(pa_ctrl_pin>=0)
		{
			...
			if (rc<0) {
				printk("Unable to read batt vol rc=%d\n", rc);
				vol=3800;
			}else{
				vol=(int)results.physical/1000;
			}
			if(vol>4100)
			{
				gpio_direction_output(pa_ctrl_pin,0);				
			}
			if(vol<4000)
			{
				gpio_direction_output(pa_ctrl_pin,1);				
			}				
		}
	}
	pr_debug("skj batt temp=%d\n",temp);
	return temp;
}
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		case POWER_SUPPLY_STATUS_FULL:
			// by skj
			rc = smb135x_charging_set(chip,false);
			if (rc < 0) {
		...
		case POWER_SUPPLY_STATUS_CHARGING:
			// by skj
			rc = smb135x_charging_set(chip,true);
			if (rc < 0) {
	...
	case POWER_SUPPLY_PROP_COOL_TEMP:
		chip->cfg_cool_bat_decidegc = val->intval;
		pr_debug("skj set COOL_TEMP=%d\n",val->intval);
		break;
	case POWER_SUPPLY_PROP_WARM_TEMP:
		chip->cfg_warm_bat_decidegc = val->intval;
		pr_debug("skj set WARM_TEMP=%d\n",val->intval);
		break;		
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = chip->current_bat_decidegc;
		pr_debug("skj get TEMP=%d\n",val->intval);
		break;
	case POWER_SUPPLY_PROP_COOL_TEMP:
		val->intval = chip->cfg_cool_bat_decidegc;
		pr_debug("skj get COOL_TEMP=%d\n",val->intval);
		break;
	case POWER_SUPPLY_PROP_WARM_TEMP:
		val->intval = chip->cfg_warm_bat_decidegc;
		pr_debug("skj get WARM_TEMP=%d\n",val->intval);
		break;		
	default:
		return -EINVAL;
	}
...
static void smb135x_external_power_changed(struct power_supply *psy)
{
	...
	// by skj
	if(current_limit>USB_MAX_CURRENT)
		current_limit=AC_MAX_CURRENT;
	if(chip->usb_present && current_limit<USB_MAX_CURRENT)
		current_limit=USB_MAX_CURRENT;
	pr_debug("skj update to %d\n", current_limit);	
...
static int otg_oc_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	...
	if (chip->otg_oc_count < MAX_OTG_RETRY) {
		// by skj
		rc = smb135x_read(chip,CMD_CHG_REG,&value);
		if (rc < 0)
			dev_err(chip->dev, "Couldn't enable  OTG mode rc=%d -1\n",	rc);
		if(value & 1){
			rc = smb135x_masked_write(chip, CMD_CHG_REG, OTG_EN, 0);
			if (rc < 0)
				dev_err(chip->dev, "Couldn't enable  OTG mode rc=%d -2\n",
					rc);			
			rc = smb135x_masked_write(chip, CMD_CHG_REG, OTG_EN, OTG_EN);
			if (rc < 0)
				dev_err(chip->dev, "Couldn't enable  OTG mode rc=%d\n",
...
static int smb135x_hw_init(struct smb135x_chg *chip)
{
	...
	// by skj
	dev_err(chip->dev, "Requesting for the HVDCP 9V\n");
	rc = smb135x_masked_write(chip, CFG_E_REG, HVDCP_5_9_BIT, HVDCP_5_9_BIT);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't request for 9V rc=%d\n", rc);
		return rc;
	}	
	...
	rc = smb135x_masked_write(chip, CFG_1A_REG,
					0xFF, 0);	
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't write cfg 0x1A rc = %d\n",
				rc);
		return rc;
	}	
#if 0
	// by skj disable AICL
	rc = smb135x_masked_write(chip, 0xD,
					(1<<2), 0);	
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't write cfg 0xB rc = %d\n",
				rc);
		return rc;
	}	
#endif	
...
static int smb_parse_dt(struct smb135x_chg *chip)
{
	...
	// by skj
	chip->vadc_dev=qpnp_get_vadc(chip->dev, "chg");
//	pr_debug("skj: vadc_dev=%d\n",(int)chip->vadc_dev);
	chip->cfg_cool_bat_decidegc=0;
	chip->cfg_warm_bat_decidegc=45;
	if (IS_ERR(chip->vadc_dev)) {
		rc = PTR_ERR(chip->vadc_dev);	
		printk("skj get vadc fail rc=%d\n",rc);
		chip->vadc_dev=0;
	}	
	pa_ctrl_pin=of_get_named_gpio(node,"qcom,pa-ctrl-pin",0);	
	if(pa_ctrl_pin>=0){
		rc=gpio_request(pa_ctrl_pin,"pa_ctrl_pin");
		if(rc<0){
			printk("skj pa_ctrl_pin request err rc=%d\n",rc);
		}
	}
	return 0;
}
...
// by skj
struct temp_to_health{
	int start;
	int end;
	int value;	
};

static struct temp_to_health convert_data[]={
	{-10000,10,POWER_SUPPLY_HEALTH_COLD},
	{-10,60,POWER_SUPPLY_HEALTH_COOL},
	{40,460,POWER_SUPPLY_HEALTH_GOOD},
	{440,510,POWER_SUPPLY_HEALTH_WARM},
	{490,10000,POWER_SUPPLY_HEALTH_OVERHEAT},
};

static int get_battery_health(int temp)
{
	static int last_level=0;
	int i;

	if(temp>=convert_data[last_level].start && temp<=convert_data[last_level].end)
		return convert_data[last_level].value;
	
	for(i=0;i<ARRAY_SIZE(convert_data);i++){
		if(temp>=convert_data[i].start && temp<=convert_data[i].end)
			last_level=i;
	}
...
static void smb135x_temp_monitor_work(struct work_struct *work) 
{
	...
	struct smb135x_chg *chip = container_of(dwork,
				struct smb135x_chg, temp_monitor_work);	
	union power_supply_propval value;
	
	int capacity=DEFAULT_BATT_CAPACITY;
	int charging=0;
	...
	//just_debug_show(chip);
 	...
 	if(POWER_SUPPLY_HEALTH_OVERHEAT==chip->health || 
		POWER_SUPPLY_HEALTH_COLD==chip->health){
			...
			smb135x_battery_set_property(&chip->batt_psy,
				POWER_SUPPLY_PROP_STATUS,&value);	
	}else{
		smb135x_charging_set(chip,true);
	}
	...
	if(POWER_SUPPLY_HEALTH_GOOD!=chip->health)
		current_ma=current_ma/2;
	
	if(charger_present){
		smb135x_set_fastchg_current(chip, current_ma);		
		wake_lock_timeout(&smb135x_wakelock,msecs_to_jiffies(CHG_CHECK_PERIOD_MS+1000));
	}
	
	value.intval=chip->health;
