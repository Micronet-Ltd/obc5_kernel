...
/* Copyright (c) 2012-2012, QRT Telecom. All rights reserved.
 *
...
int register_hardware_info(HARDWARE_ID id, char* name)
{
	...
	case SUB_CAM:
		...
		break;
	case IRIS_CAM:
		snprintf(hardware_info->iris_cam, 125,name);
		ret = device_create_file(&hardware_info_pdev->dev, &dev_attr_iris_cam);
		if (ret) {
			printk("%s:error on device_create_file %s\n", __func__, name);
		}
		break;
