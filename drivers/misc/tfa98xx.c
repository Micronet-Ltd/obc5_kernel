...
{
	int ret = 0;
	struct i2c_board_info *tfa98xx_ptr;

	tfa98xx_ptr = &tfa98xx_i2c_boardinfo;
...
#else
	//for mtk platform...
	ret = i2c_register_board_info(I2C_STATIC_BUS_NUM, tfa98xx_ptr, 1); /* ARRAYSIZE(tfa98xx_i2c_boardinfo) */
	if (ret < 0) {
		printk("i2c_register_board_info error %d\n", ret);
	}
	pr_info("howard tfa98xx i2c_register_board_info ret %d\n", ret);
#endif
