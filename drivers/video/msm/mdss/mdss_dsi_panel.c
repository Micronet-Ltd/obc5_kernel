...
/*Modifyed by Guohongtao to modify display problem when lcd wake up  2014-09-16 begin*/
int panel_name_status;
/*Modifyed by Guohongtao to modify display problem when lcd wake up  2014-09-16 end*/
//guoqifan add for hardware info 2014.06.25 start
#ifdef CONFIG_GET_HARDWARE_INFO
#include <mach/hardware_info.h>
static char tmp_panel_name[100];
#endif
//guoqifan add for hardware info 2014.06.25 end
//add by jiao.shp
#define BL_CTRL_BY_I2C_WRITE
extern int mp3309_i2c_init(void);
extern int mp3309_bl_ctrl(int);
...
static int mdss_dsi_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
    ...
    if (gpio_is_valid(ctrl_pdata->disp_vddio_en)) {
		rc = gpio_request(ctrl_pdata->disp_vddio_en,"disp_vddio_en");
		if (rc) {
			pr_err("request disp_vddio_en gpio failed, rc=%d\n",rc);
			goto disp_vddio_en_err;
		}
	}
...
rst_gpio_err:
	...
	if(gpio_is_valid(ctrl_pdata->disp_vddio_en))
		gpio_free(ctrl_pdata->disp_vddio_en);
disp_vddio_en_err:
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
    ...
    if (!gpio_is_valid(ctrl_pdata->disp_vddio_en)) {
		pr_debug("%s:%d, disp_vddio_en gpio not configured\n",
			   __func__, __LINE__);
	}
	...
	if (enable) {
		...
		if (!pinfo->cont_splash_enabled) {
            ...
            if (gpio_is_valid(ctrl_pdata->disp_vddio_en))
				gpio_direction_output((ctrl_pdata->disp_vddio_en), 1);
			...
			if (gpio_is_valid(ctrl_pdata->bklt_en_gpio)){
                #ifdef CONFIG_MP3309_DRIVER
                static bool have_init_flag = false;
                #endif
				gpio_direction_output((ctrl_pdata->bklt_en_gpio), 1);
                #ifdef CONFIG_MP3309_DRIVER
                if(have_init_flag == false){
                    mp3309_i2c_init();//add by jiao.shp
                    have_init_flag = true;
                }	
                #endif
            }
        ...
        if (gpio_is_valid(ctrl_pdata->disp_vddio_en)) {
			gpio_set_value((ctrl_pdata->disp_vddio_en), 0);
            gpio_free(ctrl_pdata->disp_vddio_en);
        }
	...
	bool cmd_cfg_cont_splash)
{
	...
	if (!strncmp(panel_name, "hongcai_sh1282g003_k499J2503_boe5p0inch_fwvga", 45)){
	    panel_name_status = 1;
...
//guoqifan add for hardware info 2014.06.25 end
/*modify by zhouwentao  for SW00073588 info 2014.08.26 */
	if (strncpy(tmp_panel_name, "sh1282",6))
	{
	//flag_SH1282G =1;
	}
    #endif
