...
#define THRES_TOLERANCE  40  //I think this is a proper value. It should not be too big.
#define THRES_DIFF    60     //I am not sure whether this is a proper value but I guess so. 
...
static  struct ltr559_reg reg_tbl[] = {
		...
		},
		{
				...
				.addr = 0x83,
				.defval = 0x01,
#ifdef CONFIG_INPUT_LTR559_P752		
				.curval = 0x0f, //byron 0x06
#else
				.curval = 0x06,
#endif				
		},
...
static void ltr559_ps_work_func(struct work_struct *work)
{
	...
	if ((data->ps_open_state == 1) && (als_ps_status & 0x02)) {
		...
		if(psdata > data->platform_data->prox_hsyteresis_threshold){
			...
			val_temp = 0;
		} else if (psdata < data->platform_data->prox_threshold){
			data->ps_state = 1; //far
			ltr559_set_ps_threshold(client, LTR559_PS_THRES_LOW_0, 0);
			ltr559_set_ps_threshold(client, LTR559_PS_THRES_UP_0, data->platform_data->prox_threshold);
			val_temp = 1;
				...
				const char *buf, size_t size)
{
	...
	// xuke @ 20140828	Use the unified interface.
...
/* add by zj begin for hal error*/
static int ltr559_ps_poll_delay(struct sensors_classdev *sensors_cdev,
...
{
	struct ltr559_data *als_ps = container_of(sensors_cdev, struct ltr559_data, ps_cdev);
	unsigned int i, tmp, ps_th_h, ps_th_l;
	int temp[3] = { 0 };

	int average = 0;
	int pre_ps_open_state=als_ps->ps_open_state;
	if(als_ps->ps_open_state==0)
	{
		ltr559_ps_set_enable(&als_ps->ps_cdev, 1);
	}
	for (i = 0; i < 20; i++) {
		msleep(20);
		tmp = ltr559_ps_read(als_ps->client);
		if (tmp < 0)
	...
	als_ps->platform_data->prox_hsyteresis_threshold = ps_th_h+THRES_DIFF;
	als_ps->platform_data->prox_threshold = ps_th_l;
	...
	if (apply_now) {
		...
		ltr559_set_ps_threshold(als_ps->client,LTR559_PS_THRES_LOW_0,ps_th_l);
		ltr559_set_ps_threshold(als_ps->client,LTR559_PS_THRES_UP_0,ps_th_h);
	}
	...
	if(pre_ps_open_state==0)
		ltr559_ps_set_enable(&als_ps->ps_cdev, 0);
		...
		struct cal_result_t *cal_result)
{
	...
	average=data->ps_cal_params[2];
	ps_th_h = average + THRES_TOLERANCE + THRES_DEFAULT_DIFF;
	ps_th_l = average + THRES_TOLERANCE;
	data->platform_data->prox_hsyteresis_threshold = ps_th_h+THRES_DIFF;
	data->platform_data->prox_threshold = ps_th_l+THRES_DIFF;
	//printk("%s,cal_result->threshold_h:%d,cal_result->threshold_l:%d, cal_result->bias:%d\n",__func__,ps_th_h,ps_th_l,average);
	ltr559_set_ps_threshold(data->client,LTR559_PS_THRES_LOW_0,ps_th_l);
	ltr559_set_ps_threshold(data->client,LTR559_PS_THRES_UP_0,ps_th_h);
...
int ltr559_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->ps_cdev.sensors_poll_delay = ltr559_ps_poll_delay;
#ifdef LTR559_PS_CALIBERATE
	data->ps_cdev.sensors_calibrate = ltr559_ps_calibrate;
	data->ps_cdev.sensors_write_cal_params = ltr559_ps_write_calibrate;
