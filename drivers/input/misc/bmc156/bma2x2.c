...
/*x,y need to be invesed becase of HW Register for BMI058*/
	...
	val->y = bsd.y;
	val->z = bsd.z;
	if (client_data->acc_use_cal) {
		val->x -= client_data->acc_cal_params[0];
		val->y -= client_data->acc_cal_params[1];
		val->z -= client_data->acc_cal_params[2];
	}
...
/* changed by shengweiguang begin 20150810 */
#if 0
...
#else
	input_report_abs(bma2x2->input, ABS_X, bma2x2->pdata->negate_x ? -(int)value->x << bma2x2->sensitivity : (int)value->x << bma2x2->sensitivity);
	input_report_abs(bma2x2->input, ABS_Y, bma2x2->pdata->negate_y ? -(int)value->y << bma2x2->sensitivity : (int)value->y << bma2x2->sensitivity);
	input_report_abs(bma2x2->input, ABS_Z, bma2x2->pdata->negate_z ? -(int)value->z << bma2x2->sensitivity : (int)value->z << bma2x2->sensitivity);
#endif
/* changed by shengweiguang end 20150810 */
		...
		int axis, int apply_now)
{
	...
	if (bma2x2_set_bandwidth(client, BMA2X2_BW_125HZ) < 0) {
		dev_err(&client->dev, "set calibrate bandwidth error\n");
		return -EINVAL;
	}
	if (bma2x2_set_range(client, BMA2X2_RANGE_SET) < 0) {
	...
	data->acc_use_cal = false;//for read no calc data
	for (i = 0; i < MPU_ACC_CAL_COUNT; i++) {
		msleep(MPU_ACC_CAL_DELAY);
		bma2x2_read_accel_xyz(data->bma2x2_client, data->sensor_type,&acc_value);
		if (i < CAL_SKIP_COUNT)
			continue;
		//bma2x2_remap_sensor_data(&acc_value, data);
		arry[0] += acc_value.x;
		arry[1] += acc_value.y;
		arry[2] += acc_value.z;
	}
	...
	case AXIS_XYZ:
		...
		break;
	default:
	printk("calibrate bma2x2 accel CMD error\n");
		ret = -EINVAL;
		goto exit;
	}

	if (apply_now) {
		data->acc_cal_params[0] = arry[0];
		data->acc_cal_params[1] = arry[1];
		data->acc_cal_params[2] = arry[2];
		data->acc_use_cal = true;
	}
	snprintf(data->acc_cal_buf, sizeof(data->acc_cal_buf),
			"%d,%d,%d", arry[0], arry[1], arry[2]);
	sensors_cdev->params = data->acc_cal_buf;
...
{
	struct bma2x2_data *sensor = container_of(sensors_cdev,
					struct bma2x2_data, cdev);
...
/* add by shengweiguang begin 20150810*/
...
/* add by shengweiguang end 20140810*/
