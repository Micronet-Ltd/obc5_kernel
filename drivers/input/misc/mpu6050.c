...
static struct wake_lock mpu6050_wakelock;
...
static int mpu6050_power_ctl(struct mpu6050_sensor *sensor, bool on)
{
	...
	if (on && (!sensor->power_enabled)) {
		pr_info("Howard mpu6050_power_ctl on\n");
		...
		pr_info("Howard mpu6050_power_ctl off\n");
		...
		pr_info("Howard Ignore power status change from %d to %d\n", on, sensor->power_enabled);
	}
	return rc;
}
...
{
	u32 shift;
	pr_info("Howard mpu6050_read_single_event\n");
	if (sensor->cfg.accel_enable) {
		pr_info("Howard mpu6050_read_single_event accel_enable\n");
	...
	if (sensor->cfg.gyro_enable) {
		pr_info("Howard mpu6050_read_single_event gyro_enable\n");
...
static irqreturn_t mpu6050_interrupt_thread(int irq, void *data)
{
	...
	int ret;
	pr_info("Howard mpu6050_interrupt_thread\n");
	...
	if (ret & BIT_FIFO_OVERFLOW)
		...
		wake_lock_timeout(&mpu6050_wakelock,msecs_to_jiffies(5000));
		pr_info("howard time out for wakeup\n");
	}
...
static void mpu6050_accel_work_fn(struct work_struct *work)
{
	...
	shift = mpu_accel_fs_shift[sensor->cfg.accel_fs];
	pr_debug("Howard mpu6050_accel_work_fn, x %d, y %d, z %d, shift %d\n", (sensor->axis.x << shift),
		(sensor->axis.y << shift), (sensor->axis.z << shift), shift);
	input_report_abs(sensor->accel_dev, ABS_X,
		(sensor->axis.x << shift));
	input_report_abs(sensor->accel_dev, ABS_Y,
		(sensor->axis.y << shift));
...
static void mpu6050_gyro_work_fn(struct work_struct *work)
{
	...
	shift = mpu_gyro_fs_shift[sensor->cfg.fsr];
	pr_debug("Howard mpu6050_gyro_work_fn, rx %d, ry %d, rz %d\n", (sensor->axis.rx >> shift), 
		(sensor->axis.ry >> shift), (sensor->axis.rz >> shift));
	input_report_abs(sensor->gyro_dev, ABS_RX,
		(sensor->axis.rx >> shift));
	input_report_abs(sensor->gyro_dev, ABS_RY,
		(sensor->axis.ry >> shift));
...
static int mpu6050_set_lpa_freq(struct mpu6050_sensor *sensor, int lpa_freq)
{
	int ret;
	u8 data;
	pr_info("Howard mpu6050_set_lpa_freq\n");
				...
				bool en, u32 mask)
{
	...
	reg = &sensor->reg;
	pr_info("Howard mpu6050_switch_engine %d\n", en);
	/*
					...
					bool power_on)
{
	...
	s32 ret;
	u8 val;
	pr_info("Howard mpu6050_set_power_mode\n");
...
static int mpu6050_gyro_enable(struct mpu6050_sensor *sensor, bool on)
{
	int ret;
	u8 data;
	pr_info("Howard mpu6050_gyro_enable\n");
...
static int mpu6050_restore_context(struct mpu6050_sensor *sensor)
{
	...
	reg = &sensor->reg;
	pr_info("Howard mpu6050_restore_context\n");
...
static void mpu6050_reset_chip(struct mpu6050_sensor *sensor)
{
	...
	client = sensor->client;
	pr_info("Howard mpu6050_reset_chip\n");
...
static int mpu6050_config_sample_rate(struct mpu6050_sensor *sensor)
{
	...
	pr_info("Howard mpu6050_config_sample_rate, delay_ms %d\n", delay_ms);
...
static void mpu6050_flush_fifo(struct mpu6050_sensor *sensor)
{
	...
	u16 *buf, cnt;
	u8 shift;
	pr_info("Howard mpu6050_flush_fifo\n");
...
static int mpu6050_gyro_cdev_flush(struct sensors_classdev *sensors_cdev)
{
	struct mpu6050_sensor *sensor = container_of(sensors_cdev,
			struct mpu6050_sensor, gyro_cdev);
	pr_info("Howard mpu6050_gyro_cdev_flush\n");
	mutex_lock(&sensor->op_lock);
	mpu6050_flush_fifo(sensor);
	input_event(sensor->gyro_dev,
...
static int mpu6050_accel_enable(struct mpu6050_sensor *sensor, bool on)
{
	...
	if (sensor->cfg.is_asleep)
		return -EINVAL;
	pr_info("Howard mpu6050_accel_enable %d\n", on);
...
static int mpu6050_accel_cdev_flush(struct sensors_classdev *sensors_cdev)
{
	struct mpu6050_sensor *sensor = container_of(sensors_cdev,
			struct mpu6050_sensor, accel_cdev);
	pr_info("Howard mpu6050_accel_cdev_flush\n");
	mutex_lock(&sensor->op_lock);
	mpu6050_flush_fifo(sensor);
	mutex_unlock(&sensor->op_lock);
	input_event(sensor->accel_dev,
		...
		int axis, int apply_now)
{
	...
	int arry[3] = { 0 };
	int i, delay_ms;
	pr_info("Howard mpu6050_accel_calibration\n");
	mutex_lock(&sensor->op_lock);
		...
		struct cal_result_t *cal_result)
{
	struct mpu6050_sensor *sensor = container_of(sensors_cdev,
			struct mpu6050_sensor, accel_cdev);
	pr_info("Howard mpu6050_write_accel_cal_params\n");
	mutex_lock(&sensor->op_lock);
...
{
	mpu6050_read_accel_data(sensor, &sensor->axis);
	pr_debug("Howard mpu6050_acc_data_process, x %d, y %d, z %d, place %d\n", (sensor->axis.x),
		(sensor->axis.y), (sensor->axis.z), sensor->pdata->place);
	mpu6050_remap_accel_data(&sensor->axis, sensor->pdata->place);
...
static int MPU6050InitMotion(struct i2c_client *sensor_i2c_client);

u32 mpu6050_int_mode;

static ssize_t mpu6050_accel_interrupt_set(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	int ret;
	...
	if (enable){
		pr_info("Howard set mpu6050 interrupt mode enable\n");
		ret = MPU6050InitMotion(sensor->client);
		if (ret) {
			pr_err("Howard MPU6050InitMotion error\n");
			return -EBUSY;
		}
		sensor->cfg.gyro_enable = 1;
		sensor->cfg.accel_enable = 1;
		...
		pr_info("Howard set mpu6050 interrupt mode disable\n");
		
		ret = mpu6050_init_config(sensor);
		if (ret) {
			pr_err("Failed to set default config\n");
			return -EBUSY;
		}
		sensor->cfg.gyro_enable = 0;
		sensor->cfg.accel_enable = 0;
...
static ssize_t mpu6050_accel_interrupt_get(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	return snprintf(buf, 4, "%d\n", mpu6050_int_mode);
}
...
static struct device_attribute accel_attr[] = {
	...
	__ATTR(enable, S_IRUGO | S_IWUSR,
		...
		mpu6050_accel_attr_set_enable),
	__ATTR(interrupt, S_IRUGO | S_IWUGO,
		mpu6050_accel_interrupt_get,
		mpu6050_accel_interrupt_set),
#ifdef DEBUG_NODE
...
{
	int rc;
	pr_info("howard mpu6050_dt_get_place\n");
	rc = mpu6050_dt_get_place(dev, pdata);
	if (rc)
		return rc;
	pr_info("howard invn,gpio-int\n");
	...
	pr_info("howard invn,gpio-en\n");
	pdata->gpio_en = of_get_named_gpio_flags(dev->of_node,
				"invn,gpio-en", 0, NULL);
	pr_info("howard invn,use-interrupt\n");
...
static int MPU6050Init(struct i2c_client *sensor_i2c_client)
{
	...
	client = sensor_i2c_client;
	
	//reset MPU
    //tempBuf[0] = 0x80;
    //Serial_Write(I2CSlaveAddr, 0x6B, 1, tempBuf, 1);
    data = 0x80;
	ret = i2c_smbus_write_byte_data(client, 0x6B, data);
	if (ret < 0)
		return ret;
    ...
    //turn on all gyros, use X gyro's MEMS oscillators as clock reference
    //tempBuf[0] = 0x02;
    //Serial_Write(I2CSlaveAddr, 0x6B, 1, tempBuf, 1);
    data = 0x02;
	ret = i2c_smbus_write_byte_data(client, 0x6B, data);
    ...
    //sample rate = internal sample frequency / (7+1) = 1KHz / 8 = 125Hz
    //tempBuf[0] = 0x07;
    //Serial_Write(I2CSlaveAddr, 0x19, 1, tempBuf, 1);
    data = 0x07;
	ret = i2c_smbus_write_byte_data(client, 0x19, data);
	if (ret < 0)
		return ret;

    //DLPF = 0x02, acc = 94HZ, gyro = 98HZ
    //tempBuf[0] = 0x03;
    //Serial_Write(I2CSlaveAddr, 0x1A, 1, tempBuf, 1);
    data = 0x03;
	ret = i2c_smbus_write_byte_data(client, 0x1A, data);
	if (ret < 0)
		return ret;

    //full scale range = 2000dps
    //tempBuf[0] = 0x18;
    //Serial_Write(I2CSlaveAddr, 0x1B, 1, tempBuf, 1);
    data = 0x18;
	ret = i2c_smbus_write_byte_data(client, 0x1B, data);
	if (ret < 0)
		return ret;

    //full scale range = +-4G
    //Serial_Read(I2CSlaveAddr, 0x1C, 1, &tempBuf[0], 1);
    //tempBuf[0] |= 0x08;
    //Serial_Write(I2CSlaveAddr, 0x1C, 1, tempBuf, 1);
    ret = i2c_smbus_read_byte_data(client, 0x1C);
	if (ret < 0)
		return ret;
	data = (u8)ret;
	data |= 0x08;
	ret = i2c_smbus_write_byte_data(client, 0x1C, data);
	if (ret < 0)
		return ret;
	

    //interrupt active high, push-pull, latch until clear, clear if any register read
    //tempBuf[0] = 0x30;
    //Serial_Write(I2CSlaveAddr, 0x37, 1, tempBuf, 1);
    data = 0x30;
	ret = i2c_smbus_write_byte_data(client, 0x37, data);
	if (ret < 0)
		return ret;

    //trigger while data ready
    //tempBuf[0] = 0x01;
    //Serial_Write(I2CSlaveAddr, 0x38, 1, tempBuf, 1);
    data = 0x01;
	ret = i2c_smbus_write_byte_data(client, 0x38, data);
	if (ret < 0)
		return ret;
...
static int Enable_MPU6050_MotionDetect(struct i2c_client *sensor_i2c_client)
{
    //unsigned char tempBuf[20];
	u8 data;
	s32 ret;
    ...
    //enable the motion detection interupt 
    //Serial_Read(I2CSlaveAddr, 0x38, 1, &tempBuf[0], 1);
    //tempBuf[0] |= 0xE0;
    //tempBuf[0] = 0xE0;
    //Serial_Write(I2CSlaveAddr, 0x38, 1, tempBuf, 1);
    ret = i2c_smbus_read_byte_data(client, 0x38);
	if (ret < 0)
		return ret;
	data = (u8)ret;
	data |= 0xE0;
    ...
    //set MOT_COUNT bits of 0x00, reset count if lower than theshold
    //set FF_COUNT bits of 0x00, reset count if higher than theshold
    //Serial_Read(I2CSlaveAddr, 0x69, 1, &tempBuf[0], 1);
    //tempBuf[0] &= 0xF0;
    //Serial_Write(I2CSlaveAddr, 0x69, 1, tempBuf, 1);
    ret = i2c_smbus_read_byte_data(client, 0x69);
	if (ret < 0)
		return ret;
	data = (u8)ret;
	data &= 0xF0;
	ret = i2c_smbus_write_byte_data(client, 0x69, data);
	if (ret < 0)
		return ret;
    
    //motion duration = 2*1ms
    //motion threshold = 2*2mg
    //tempBuf[0] = 2;
    //Serial_Write(I2CSlaveAddr, 0x1F, 1, tempBuf, 1);
    data = 2;
	ret = i2c_smbus_write_byte_data(client, 0x1F, data);
	if (ret < 0)
		return ret;
    //tempBuf[0] = 2;
    //Serial_Write(I2CSlaveAddr, 0x20, 1, tempBuf, 1);
    data = 2;
	ret = i2c_smbus_write_byte_data(client, 0x20, data);
	if (ret < 0)
		return ret;
    
    //zero motion duration  = 50*64ms
    //zero motion threshold = 1*2mg
    //tempBuf[0] = 1;
    //Serial_Write(I2CSlaveAddr, 0x21, 1, tempBuf, 1);
    data = 1;
	ret = i2c_smbus_write_byte_data(client, 0x21, data);
	if (ret < 0)
		return ret;
    //tempBuf[0] = 50;
    //Serial_Write(I2CSlaveAddr, 0x22, 1, tempBuf, 1);
    data = 50;
	ret = i2c_smbus_write_byte_data(client, 0x22, data);
	if (ret < 0)
		return ret;
    
    //free fall duration  = 20*1ms
    //free fall threshold = 10*2mg
    //tempBuf[0] = 20;
    //Serial_Write(I2CSlaveAddr, 0x1D, 1, tempBuf, 1);
    data = 20;
	ret = i2c_smbus_write_byte_data(client, 0x1D, data);
	if (ret < 0)
		return ret;
    //tempBuf[0] = 20;
    //Serial_Write(I2CSlaveAddr, 0x1E, 1, tempBuf, 1);
    data = 20;
	ret = i2c_smbus_write_byte_data(client, 0x1E, data);
	if (ret < 0)
		return ret;

	return 0;
}
static int MPU6050_LPA_Init(struct i2c_client *sensor_i2c_client)
{
    //unsigned char tempBuf[20];
    u8 data;
	s32 ret;
    ...
    //set DLPF bits to 0x03
    //Serial_Read(I2CSlaveAddr, 0x1A, 1, &tempBuf[0], 1);
    //tempBuf[0] &= 0xF8;
    //tempBuf[0] |= 0x03;
    //Serial_Write(I2CSlaveAddr, 0x1A, 1, tempBuf, 1);
    ret = i2c_smbus_read_byte_data(client, 0x1A);
	if (ret < 0)
		return ret;
	data = (u8)ret;
	data &= 0xF8;
    ...
    //setup the frequency of wakeup
    //put gyro in standby
    //tempBuf[0] = 0xC7;	//40HZ
    //tempBuf[0] = 0x87;	//20HZ
    //tempBuf[0] = 0x47;	//5HZ
    //tempBuf[0] = 0x07;		//1.25HZ
    //Serial_Write(I2CSlaveAddr, 0x6C, 1, tempBuf, 1);
    data = 0x07;
	ret = i2c_smbus_write_byte_data(client, 0x6C, data);
	if (ret < 0)
		return ret;
    
    //set the cycle bit to be 1
    //set the temp dis bit to disable temperature sensor
    //use internal 8MHZ oscillators as clock reference
    //tempBuf[0] = 0x28;
    //Serial_Write(I2CSlaveAddr, 0x6B, 1, tempBuf, 1);
    data = 0x28;
	ret = i2c_smbus_write_byte_data(client, 0x6B, data);
...
static int MPU6050InitMotion(struct i2c_client *sensor_i2c_client)
{
    ...
    u8 data;
	s32 ret;
	pr_info("Howard MPU6050InitMotion\n");
	client = sensor_i2c_client;
	...
	pr_info("Howard read addr(0x6C)=0x%x\n", ret);
	ret = i2c_smbus_read_byte_data(client, 0x6B);
	pr_info("Howard read addr(0x6B)=0x%x\n", ret);
				   ...
				   const struct i2c_device_id *id)
{
	...
	struct mpu6050_platform_data *pdata;
	int ret;
	pr_info("Howard mpu6050_probe\n");
	wake_lock_init(&mpu6050_wakelock, WAKE_LOCK_SUSPEND,"mpu6050_wakelock");
	...
	pr_info("Howard 6050 i2c_set_clientdata\n");
	sensor->client = client;
	sensor->dev = &client->dev;
	i2c_set_clientdata(client, sensor);
	pr_info("Howard 6050 devm_kzalloc\n");
	if (client->dev.of_node) {
		...
		pr_info("Howard 6050 mpu6050_parse_dt\n");
	...
	pr_info("Howard 6050 mpu6050_pinctrl_init\n");
	...
	pr_info("Howard 6050 mpu_check_chip_type\n");
	...
	pr_info("Howard 6050 devm_input_allocate_device\n");
	...
	if ((sensor->pdata->use_int) &&
		...
		ret = request_threaded_irq(client->irq,
				     ...
				     "mpu6050", sensor);
		pr_info("Howard int_flags %d, ret %d\n", sensor->pdata->int_flags, ret);
		if (ret) {
			dev_err(&client->dev,
	...
	pr_info("Howard 6050 input_register_device\n");
	...
	pr_info("Howard probe 6050 success\n");
	return 0;
...
static int mpu6050_suspend(struct device *dev)
{
	...
	struct mpu6050_sensor *sensor = i2c_get_clientdata(client);
	int ret = 0;
	pr_info("Howard mpu6050_suspend\n");
	mutex_lock(&sensor->op_lock);
	if ((sensor->batch_accel) || (sensor->batch_gyro)) {
		mpu6050_set_interrupt(sensor,
				BIT_FIFO_OVERFLOW, false);
	...
	if (sensor->motion_det_en) {
		...
		if (ret == 0){
			...
			pr_info("Howard mpu6050_set_motion_det\n");
		}
#endif
