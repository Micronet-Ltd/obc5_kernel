...
//modify by chenchen for gesture 20140721 begin				
 ...
 //modify by chenchen for gesture 20140721 end              
            ...
            else
            {
                ...
                doze_buf[2] = 0x00;
                gtp_i2c_write(i2c_connect_client, doze_buf, 3);
//modify by chenchen for tp crash 20140925
               // gtp_enter_doze(ts);
            }
        }
        if (ts->use_irq)
...
static s32 gtp_get_info(struct goodix_ts_data *ts)
{
    ...
    GTP_INFO("%s:X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
            __func__,ts->abs_x_max,ts->abs_y_max,ts->int_trigger_type);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++)
		GTP_INFO("config_data(%d) Lengths: %d",
	...
	if (i == GOODIX_MAX_CFG_GROUP) {
        ...
        GTP_INFO("%d,Sensor_ID: %d",__LINE__,sensor_id);
    }
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
            GTP_INFO("cfg-data%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X", sensor_id, 
...
//open by chenchen 20140514
...
#endif
    {
        ...
        config[ts->gtp_cfg_len] = (~check_sum) + 1;
    #endif
        GTP_INFO("%s:X_MAX: %d, Y_MAX: %d, TRIGGER: 0x%02x",__func__,ts->abs_x_max,ts->abs_y_max,ts->int_trigger_type);
    }
...
//modified by chenchen 20140610 begin			
			//return 1;
			pdata->module_name[i] = "unknow";
//modified by chenchen 20140610 end
 			}
		}
...
//modify by chenchen for tp crash 20140925 begin
	 if (ts->use_irq)
       	 gtp_irq_disable(ts);
   	 else
       	 hrtimer_cancel(&ts->timer);
