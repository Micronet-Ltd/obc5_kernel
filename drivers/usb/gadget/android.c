...
#include "f_ncm.c"
#include "f_charger.c"
int qrt_mass_flag = 0;// added by shengweiguang for usb mass_storage sd-card flag ql_1000 2015/8/17
int qrt_mdm_usb_mng_enable = 0; // add by shengweiguang for J20 and J20TMP mdm
int qrt_mdm_usb_otg_mng_enable = 0; // add by shengweiguang for J20 and J20TMP mdm
...
//deleted by shengweiguang 20160817 begin
/*
	config->fsg.nluns = 1;
...
*/
//deleted by shengweiguang 20150817 end
//added by shengweiguang 20150817 begin
	config->fsg.nluns = 0;
	snprintf(name[1], MAX_LUN_NAME, "lun");
	config->fsg.luns[1].removable = 1;
    config->fsg.luns[1].nofua = 1; // Added by shengweiguang for usb speed. (general) 2015-8-17
	...
	config->fsg.nluns = 2;
//added by shengweiguang 20150817 end
	...
	for (i = 0; i < uicc_nluns; i++) {
        ...
        config->fsg.luns[n].nofua = 1; // Added by shengweiguang for usb speed. (general) 2015-8-17
		config->fsg.nluns++;
	}
...
/* add by shengweiguang for J20 and J20tmp for usb lock begin */
static ssize_t mdm_usb_mng_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
	//struct android_dev *dev = dev_get_drvdata(pdev);

	return snprintf(buf, PAGE_SIZE, "%d\n", qrt_mdm_usb_mng_enable);
}

static ssize_t mdm_usb_mng_store(struct device *pdev, struct device_attribute *attr,
			    const char *buff, size_t size)
{
	sscanf(buff, "%d", &qrt_mdm_usb_mng_enable);
	//qrt_mdm_usb_mng_enable = 0;// add open usb device for J20 and J20tmp temp
	return size;
}

static ssize_t mdm_usb_otg_mng_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{

	return snprintf(buf, PAGE_SIZE, "%d\n", qrt_mdm_usb_otg_mng_enable);
}

static ssize_t mdm_usb_otg_mng_store(struct device *pdev, struct device_attribute *attr,
			    const char *buff, size_t size)
{
	sscanf(buff, "%d", &qrt_mdm_usb_otg_mng_enable);
	return size;
}
/* add by shengweiguang for J20 and J20tmp for usb lock end */
...
/*added by shengweiguang for usb mass_storage sd-card flag ql_1000 2015/8/17 */
	if(!strcmp(buf, "diag,serial,mass_storage") || !strcmp(buf, "diag,serial,mass_storage,adb"))
	    {
	          qrt_mass_flag = 1;
	    }
	else 
...
/* add by shengweiguang for J20 and J20tmp for usb lock begin */
static DEVICE_ATTR(mdm_usb_mng_enable, S_IRUGO | S_IWUSR,
		mdm_usb_mng_show, mdm_usb_mng_store);
static DEVICE_ATTR(mdm_usb_otg_mng_enable, S_IRUGO | S_IWUSR,
		mdm_usb_otg_mng_show, mdm_usb_otg_mng_store);
/* add by shengweiguang for J20 and J20tmp for usb lock end */
...
static struct device_attribute *android_usb_attributes[] = {
	...
	&dev_attr_mdm_usb_mng_enable,   // add by shengweiguang for J20 mdm
	&dev_attr_mdm_usb_otg_mng_enable, // add by shengweiguang for J20 mdm
	NULL
};
