...
extern volatile int qrt_mdm_usb_otg_mng_enable; 
...
static struct device *dev_bak;
static char *id_bak;
static int usb_id_gpio_num=(-1);
struct regulator *devm_regulator_get_smb1358(struct device *dev, const char *id)
{
	dev_bak=dev;
	id_bak=(char *)id;
...
void control_vbus_by_id(void)
{
	int value=0;
	int is_enable=0;
	int ret;

	if(IS_ERR(vbus_otg) || 0==vbus_otg)	
		vbus_otg=devm_regulator_get_smb1358(dev_bak,id_bak);
	if(IS_ERR(vbus_otg))
		return;
	if(usb_id_gpio_num<0)
		return;
	value=gpio_get_value(usb_id_gpio_num);	
	is_enable=regulator_is_enabled(vbus_otg);	
	if(value && is_enable)
...
int regulator_enable_smb1358(struct regulator *regulator)
{
	control_vbus_by_id();
	return 0;
}

int regulator_disable_smb1358(struct regulator *regulator)
{
	control_vbus_by_id();
	return 0;
}
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	if (!msm_bam_usb_lpm_ok(CI_CTRL)) {
        ...
        dev_notice(phy->dev, "lpm failure\n");
		return -EBUSY;
	}
...
phcd_retry:
	...
	if (cnt >= phy_suspend_timeout) {
		if (phcd_retry_cnt > PHY_SUSPEND_RETRIES_MAX) {
			...
			ret = -EBUSY;
            dev_notice(phy->dev, "phy failure\n");
		...
		if (device_bus_suspend) {
			...
			ret = -EBUSY;
            dev_notice(phy->dev, "phy aborted\n");
			goto phy_suspend_fail;
		} else {
			if (phcd_retry_cnt++ < PHY_SUSPEND_RETRIES_MAX) {
	...
	dev_notice(phy->dev, "USB in low power mode\n");
...
// by skj		
#ifdef CONFIG_SMB135X_CHARGER
		vbus_otg = devm_regulator_get_smb1358(motg->phy.dev, "vbus_otg");
#else		
...
static void msm_otg_init_sm(struct msm_otg *motg)
{
	...
	case USB_OTG:
		if (pdata->otg_control == OTG_USER_CONTROL) {
			if (pdata->default_mode == USB_HOST) {
				...
				set_bit(ID, &motg->inputs);
                pr_notice("USB_OTG OTG_USER_CONTROL USB_PERIPHERAL B_SESS_VLD\n");
			...
			if (otgsc & OTGSC_BSV) {
                pr_notice("USB_OTG OTG_PHY_CONTROL OTGSC_BSV B_SESS_VLD\n");
				set_bit(B_SESS_VLD, &motg->inputs);
            } else
                clear_bit(B_SESS_VLD, &motg->inputs);
		} else if (pdata->otg_control == OTG_PMIC_CONTROL) {
	...
	case USB_PERIPHERAL:
		...
		if (pdata->otg_control == OTG_PHY_CONTROL) {
			if (otgsc & OTGSC_BSV) {
                pr_notice("USB_PERIPHERAL OTG_PHY_CONTROL OTGSC_BSV B_SESS_VLD\n");
				set_bit(B_SESS_VLD, &motg->inputs);
			} else
				clear_bit(B_SESS_VLD, &motg->inputs);
		} else if (pdata->otg_control == OTG_PMIC_CONTROL) {
			...
			set_bit(ID, &motg->inputs);
			set_bit(B_SESS_VLD, &motg->inputs);
            pr_notice("USB_PERIPHERAL OTG_USER_CONTROL OTGSC_BSV B_SESS_VLD\n");
		}
		break;
	default:
		break;
	...
	if ((otgsc & OTGSC_IDIS) && (otgsc & OTGSC_IDIE)) {
		...
		if (otgsc & OTGSC_BSV) {
			...
			msm_otg_dbg_log_event(&motg->phy, "BSV SET",
					motg->inputs, otg->phy->state);
            pr_notice("OTGSC_BSV B_SESS_VLD\n");
			set_bit(B_SESS_VLD, &motg->inputs);
		} else {
...
/* add by shengweiguang for J20tmp mdm begin */
		if(qrt_mdm_usb_otg_mng_enable)
		{
			pr_err("shengweiguang OTG ID IRQ workground for J20tmp!!!!!!\n");
			return IRQ_HANDLED;
		}
/* add by shengweiguang for J20tmp mdm end */
				...
				size_t count, loff_t *ppos)
{
	...
	case USB_PERIPHERAL:
		...
		case OTG_STATE_A_SUSPEND:
			set_bit(ID, &motg->inputs);
            pr_notice("USB_PERIPHERAL OTG_STATE_A_SUSPEND B_SESS_VLD\n");
			set_bit(B_SESS_VLD, &motg->inputs);
			break;
		default:
			goto out;
...
#ifdef CONFIG_SMB135X_CHARGER	
	usb_id_gpio_num=pdata->usb_id_gpio;
#endif
