	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (is_compat_task()) {
		...
		/* add by shengweiguang for compat mode begin 20151026 */
		strlcpy(slave_info->sensor_module_info, setting32.sensor_module_info,
			sizeof(slave_info->sensor_module_info));
		slave_info->sensor_gpio_id = setting32.sensor_gpio_id;
		slave_info->power_down_mode = setting32.power_down_mode;
		/* add by shengweiguang for compat mode end 20151026 */
	} else
#endif
	{
...
// added by yangze for camera hardware info and camera gpio id (ql1001) 2014-06-10 end 
	/*Add Begin: by hanjianfeng for camera power control 20140809*/
        if(slave_info->power_down_mode)
		s_ctrl->sensordata->power_down_mode = slave_info->power_down_mode; 
        else
		s_ctrl->sensordata->power_down_mode = POWERDOWN_DEFAULT;
	...
	}else if(slave_info->sensor_init_params.position == IRIS_CAMERA_B){
		if(slave_info->sensor_module_info){
			register_hardware_info(IRIS_CAM, slave_info->sensor_module_info);
		}else{
			pr_err("%s hardware info is NULL.\n",slave_info->sensor_name);
		}
	}else{
