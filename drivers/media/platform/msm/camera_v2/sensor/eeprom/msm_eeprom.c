...
// added by yangze for eeprom sysfs (ql1700) 2014-08-29
...
// added by yangze for eeprom sysfs (ql1700) 2014-08-29 begin
static ssize_t msm_eeprom_raw_data_show(struct device *dev,
...
{	
	...
	for (j = 0; j < e_ctrl->cal_data.num_data; j++){
		sprintf((buf+j*3),"%02x ",e_ctrl->cal_data.mapdata[j]);
		CDBG("memory_data[%d] = 0x%02x\n", j,
			e_ctrl->cal_data.mapdata[j]);
	}
	return strlen(buf);
}

static DEVICE_ATTR(raw_data, 0644,msm_eeprom_raw_data_show, NULL);


static struct attribute *msm_eeprom_attributes[] = {
	&dev_attr_raw_data.attr,
	NULL
};

static struct attribute_group msm_eeprom_attribute_group = {
	.attrs = msm_eeprom_attributes
};
// added by yangze for eeprom sysfs (ql1700) 2014-08-29 end
	...
	void __user *arg)
{
	...
	/* delete by shengweiguang for camera HAL 20151117
	if (!rc) {
		kfree(e_ctrl->cal_data.mapdata);
		kfree(e_ctrl->cal_data.map);
...
// added by yangze for eeprom sysfs (ql1700) 2014-08-29 begin	
	rc = sysfs_create_group(&pdev->dev.kobj,&msm_eeprom_attribute_group);
	if(rc<0){
		rc = 0;
	}	
// added by yangze for eeprom sysfs (ql1700) 2014-08-29 end
...
// added by yangze for eeprom sysfs (ql1700) 2014-08-29 begin	
	sysfs_remove_group(&pdev->dev.kobj, &msm_eeprom_attribute_group);
// added by yangze for eeprom sysfs (ql1700) 2014-08-29 end	
